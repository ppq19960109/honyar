...
)
{
	...
	{
		/*事件列表为空，直接添加事件*/
		HY_DEBUG("The event list is empty.\n");
		iRet = 0;
	...
	while(pstHead != pstPtr)
	{
		...
		!strncmp(pstPtr->acKey, pcKey, KEY_MAX_LEN))
		{
			...
			if(1 == pstPtr->iProtect)
			{
				...
				HY_DEBUG("scene_sync_event_req_add: %s-%s: %d\n", 
					pstPtr->acDevId, pstPtr->acKey, pstPtr->aiValue[pstPtr->iValueTotal - 1]);
				HY_DEBUG("event total: %d\n", pstPtr->iValueTotal);
				HY_DEBUG("event uiSendFlag: %x\n", pstPtr->uiSendFlag);
			}
			else
			{
				/*保护机制未开启,添加当前指令*/
...
scene_sync_event_req_add_exec:	
	...
	if(0 == ucFoundFlag)
	{
		...
		if(0 != iRet)
		{
			HY_ERROR("Sync List inst failed.\n");
...
)
{
	...
	*/
	...
	cJSON_Minify(acBuff);
	cJSON_Delete(pJson);
	HY_DEBUG("Sync exec: %s\n", acBuff);
	// iRet = hyMsgProcess_AsynQuery(acBuff,BUFF_MAX_LEN);
	iRet = hylink_scene_send(acBuff,BUFF_MAX_LEN);
...
scene_sync_event_req_exec(void)
{
	...
	while(pstHead != pstPtr)
	{
		...
		if(1 == pstPtr->iProtect)
		{
			...
			{
				HY_DEBUG("Delay send. Now = %d, Active = %d\n", stNow.tv_sec, pstPtr->stActiveTime.tv_sec);
