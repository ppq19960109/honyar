			...
			else
			{
				...
				if(0 == atoi(pstCond->acRepeat))
				{
					...
					*则该条件将永远不可能为真，所以该条件应被置为无效*/
					...
					/*将时间条件置为失效*/
					DEBUG("Set the time condition to lose.\n");
				}
			}
			pTmp = NULL;
		}			
...
{
	DEBUG("_scene_continue_trigger_timer_fun\n");
...
{
	DEBUG("scene_event_cond_true_init\n");
	...
	DEBUG("CurrNewCondTrue = %d, CurrOldCondTrue = %d\n", iNewCondTrue, iOldCondTrue);
	...
	*/
	...
	DEBUG("CurrNewCondSetTrue = %d, CurrOldCondSetTrue = %d\n", iNewCondSetTrue, iOldCondSetTrue);
	...
	*/
	...
	DEBUG("CurrNewCondListTrue = %d, CurrOldCondListTrue = %d\n", iNewCondListTrue, iOldCondListTrue);
	/*
	*首先更新条件列表的真假
	...
	*/
	...
	DEBUG("CurrNewSceneTrue = %d, CurrOldSceneTrue = %d\n", iNewSceneTrue, iOldSceneTrue);
	/*
	*首先更新条件列表的真假
	*/
...
)
{
	...
	{
		DEBUG("Cond True is Changed, CurrNewCondTrue = %d, CurrOldCondTrue = %d\n", iNewCondTrue, iOldCondTrue);
		/*如果条件的真假发生改变
		*首先更新条件的真假
		*其次由于条件的真假发生改变，所以必须重新判断该条件所在的条件集合的真假
		*/
		...
		{
			DEBUG("Cond Set True is Changed, CurrNewCondSetTrue = %d, CurrOldCondSetTrue = %d\n", iNewCondSetTrue, iOldCondSetTrue);
			/*如果条件集合的真假发生改变
			*首先更新条件集合的真假
			*其次由于条件集合的真假发生改变，所以必须重新判断该条件集合所在的条件列表的真假
			*/
			...
			{
				DEBUG("Cond List True is Changed, CurrNewCondListTrue = %d, CurrOldCondListTrue = %d\n", iNewCondListTrue, iOldCondListTrue);
				/*如果条件列表的真假发生改变
				*首先更新条件列表的真假
				*其次由于条件列表的真假发生改变，所以必须重新判断该条件列表所在的场景条件的真假
				*/
				...
				*/
				...
				{
					DEBUG("Scene Cond True is Changed, CurrNewSceneTrue = %d, CurrOldSceneTrue = %d\n", iNewSceneTrue, iOldSceneTrue);
					/*如果场景条件列表的真假发生改变
					*首先更新条件列表的真假
					*/
					pstScene->ucSceneCondTrue = iNewSceneTrue;
				...
				{
					DEBUG("Scene Cond True not Changed, CurrSceneTrue = %d\n", iNewSceneTrue);
				}
			}
			else
			{
				DEBUG("Cond List True not Changed, CurrCondListTrue = %d.\n", iNewCondListTrue);
			}
		}
		else
		{
			DEBUG("Cond Set True not Changed, CurrCondSetTrue = %d.\n", iNewCondSetTrue);
		}
		...
		if(iNewCondTrue && !iOldCondTrue && pstScene->ucSceneCondTrue)
		{
			...
			{
				DEBUG("Activation timer, Timeout %ss\n", pstCondNode->stCond.acContinueTime);
...
{
	DEBUG("scene_time_cond_true_init\n");
...
)
{
	...
	if(iNewCondTrue != iOldCondTrue)
	{
		...
		*/
		...
		{
			DEBUG("Cond Set True not Changed, CurrCondListTrue = %d.\n", iNewCondSetTrue);
		}
		/*条件由假变真，且整个场景条件为真，则执行场景*/
		if(iNewCondTrue && !iOldCondTrue && pstScene->ucSceneCondTrue)
		{
	...
	{
		DEBUG("Cond True not Changed.\n");
		DEBUG("CurrTriggerType = %s\n", pstCondNode->stCond.acTriggerType);
		...
		if(1 == iEveryTrigger && iNewCondTrue && pstScene->ucSceneCondTrue)
		{
			...
			{
				DEBUG("TriggerInterval(%d) not reached.\n", pstCondNode->stCond.iTriggerInterval);
				pstCondNode->stCond.iTriggerInterval --;
			}
		}
	}
...
{
	DEBUG("scene_event_cond_set_true_init\n");
	int iNewTrue = 0;
	int iOldTrue = 0;
	...
	*/
	...
	DEBUG("CurrNewSceneCondTrue = %d, CurrOldSceneCondTrue = %d\n", iNewTrue, iOldTrue);
	/*如果场景条件列表的真假发生改变
	*首先更新条件列表的真假
	*/
...
{
	DEBUG("scene_time_cond_set_true_init\n");
	int iNewTrue = 0;
	int iOldTrue = 0;
...
)
{
	...
	if(ERROR_SUCCESS != iRet)
	{
		...
		if(ERROR_SUCCESS != iRet)
		{
			ERROR("Create DB Table(%s) failed.\n",
				DB_TABLE_NAME_SCENE);
		}
	}
...
)
{
	...
	if(ERROR_SUCCESS != iRet)
	{
		ERROR("INSERT DB Table(%s) failed.\n", 
			DB_TABLE_NAME_SCENE);
		return ERROR_ADD;
	}
...
)
{
	...
	if(ERROR_SUCCESS != iRet)
	{
		ERROR("DELETE DB Table(%s) failed.\n", 
			DB_TABLE_NAME_SCENE);
		return ERROR_DEL;
	}
...
)
{
	...
	if(ERROR_SUCCESS != iRet)
	{
		ERROR("UPDATE DB Table(%s) failed.\n", 
			DB_TABLE_NAME_SCENE);
		return ERROR_ADD;
	}
...
)
{
	...
	if(ERROR_SUCCESS != iRet)
	{
		ERROR("The Table(%s) not exist.\n", 
			DB_TABLE_NAME_SCENE);
		return ERROR_GET;
	}

	if(ERROR_SUCCESS == iRow)
	{
		ERROR("The Scene Info(%s %s) not found.\n",
			pstInfo->acSceneId);
...
)
{
	...
	if(ERROR_SUCCESS == iRow)
	{
		ERROR("The Scene Bind Info(%s %s %s) not found.\n",
			pstPanelInfo->acDevId,
			pstPanelInfo->acKey,
			pstPanelInfo->acValue
		);
...
scene_list_lock(scene_list_class_t *_this)
{
	...
	mutex_lock(pPrivateParam->mutex);
}
/*解锁场景*/
static void 
scene_list_unlock(scene_list_class_t *_this)
{
	...
	mutex_unlock(pPrivateParam->mutex);
}
...
)
{
	...
	//修改：将原来的添加修改功能改为单一的添加功能
	{
			...
			pNewNode->stSceneInfo.stOrCond.pstTimeCondList)
		{
			...
			ERROR("New the 'or' cond set of time error.\n");
			return NULL;
		}
			...
			pNewNode->stSceneInfo.stOrCond.pstEventCondList)
		{
			...
			ERROR("New the 'or' cond set of event error.\n");
			return NULL;
		}
			...
			pNewNode->stSceneInfo.stAndCond.pstTimeCondList)
		{
			...
			ERROR("New the 'and' cond set of time error.\n");
			return NULL;
		}
			...
			pNewNode->stSceneInfo.stAndCond.pstTimeCondList)
		{
			...
			ERROR("New the 'and' cond set of event error.\n");
			return NULL;
		}
		/*初始化动作链表*/
		...
		if(ERROR_SUCCESS != iRet)
		{
			ERROR("DB Scene Info set failed.\n");
			return NULL;
		}
...
)
{
	...
	if(ERROR_SUCCESS != iRet)
	{
		ERROR("DB Scene Info del failed.\n");
		return ERROR_DEL;
	}
...
)
{
	...
	if(NULL == pstCurrScene->pstPanel)
	{
		...
		for(i = 0; i < iCount; ++i)
		{
			...
			INFO("Scene Bind The Panel(%s %s %s)\n",
	...
	while(p)
	{
		...
		if(!iFlag)
		{
			...
			INFO("Scene Unbind The Panel(%s %s %s)\n",
				pastPanelInfo[i].acDevId,
				pastPanelInfo[i].acKey,
				pastPanelInfo[i].acValue
			);
...
)
{
	...
	if(NULL != pHashValue)
	{
		...
		)
		{
			...
			if(ERROR_SUCCESS != iRet)
			{
				ERROR("DB SceneBindInfo Add failed.\n");
				return ERROR_DEL;
			}
		}
		return ERROR_SUCCESS;
...
)
{
	...
	if(NULL != pHashValue)
	{
		...
		if(ERROR_SUCCESS != iRet)
		{
			ERROR("DB SceneBindInfo del failed.\n");
			return ERROR_DEL;
		}
	...
	{
		ERROR("Not Found The Panel Info\n");
		return ERROR_NOT_FOUND;
	}
	return iRet;
}
...
)
{
	...
	)
	{
		...
		for(i = 0; i < iCount; ++i)
		{
			...
			INFO("Add Time Or Cond(%s)\n",
				pastCondInfo[i].acTimeKey);
		}
		return ERROR_SUCCESS;
	}
	if(ERROR_SUCCESS == iCount)
	{
		_this->time_or_cond_clear(_this, pstCurrScene, 1);
		INFO("Clear Time Or Cond\n");
		return ERROR_SUCCESS;
	}
	char *pacFoundFlag = 
		(char *)calloc(iCount, sizeof(char));
...
)
{
	...
	if(!strncmp(pstCondInfo->acKey, JSON_VALUE_KEY_MANUAL, KEY_MAX_LEN))
	{
		...
		if(0 == pstCurrScene->cManualTriggerFlag)
		{
			...
			if(ERROR_SUCCESS != iRet)
			{
				ERROR("Failed to add manual trigger condition.\n");
				return iRet;
			}
			...
			INFO("Add manual activation conditions for the scene(%s)\n", pstCurrScene->acSceneId);
		}
...
)
{
	...
	if(!strncmp(pstCondInfo->acKey, JSON_VALUE_KEY_MANUAL, KEY_MAX_LEN))
	{
		...
		if(1 == pstCurrScene->cManualTriggerFlag)
		{
			...
			if(ERROR_SUCCESS != iRet)
			{
				ERROR("Failed to del manual trigger condition.\n");
				return iRet;
			}
...
)
{
	...
	INFO("Scene(%s) join the execution queue\n", pstInfo->acSceneId);
	return ERROR_SUCCESS;
}
		...
		while(p)
		{
			...
			if(!strcmp(pstCondNode->stCond.acTriggerType, "StateSync"))
			{
				...
				if(strncmp(pstCond->acLastValue, pstEventInfo->acValue, VALUE_MAX_LEN))
				{
					...
					{
						/*请求回应，不做处理*/
						DEBUG("The event is a response.\n");
					}
					else
					{
						/*主动上报*/
						DEBUG("The event is a report.\n");
						/*获取需要同步的设备属性*/
...
scene_list_class_t* new_scene_list(DB *pDb)
{
	...
	*/
	...
	/*初始化互斥锁*/
	pPrivateParam->mutex = mutex_lock_create();
	if(NULL == pPrivateParam->mutex)
	{
		ERROR("The mutex creation failed.\n");
		free(pNew);
        return NULL;
	}
	/*创建哈希表*/
	...
	{
		ERROR("new_hash_table Error.\n");
		mutex_lock_destroy(pPrivateParam->mutex);
		free(pNew);
        return NULL;
	}
...
int destroy_scene_list(scene_list_class_t *_this)
{
	...
	scene_list_private_param_t *pPrivateParam = 
		(scene_list_private_param_t *)_this->acScenelistPrivateParam;
	/*取消线程*/
	pthread_cancel(pPrivateParam->pid_exec);
	pthread_cancel(pPrivateParam->pid_cond);
	/*销毁哈希表*/
	destroy_ptr_hash_table(pPrivateParam->pHashTable);
	/*销毁队列*/
	destroy_link_list(pPrivateParam->pstExecQueue);
