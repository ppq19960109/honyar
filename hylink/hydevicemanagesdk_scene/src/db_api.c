...
db_sql_exec_repetition:
	...
	{
		ERROR("The sql(%s) failed to execute: %s(%d)", pcSql, pcDbErrMsg, iRet);
...
db_get_repetition:	
	...
	{  
		ERROR("The sql(%s) failed to execute: %s(%d)\n", pcSql, pcDbErrMsg, iRet);
...
db_server_start(const char *pcPath)
{
	...
	if(SQLITE_OK != iRet)
	{
		ERROR("Open DB(%s) failed :%s", pcPath, sqlite3_errmsg(pDb->fp));
		return NULL;
	}
...
db_server_stop(DB* db)
{
	if(NULL == db)
	{
		ERROR("Parameter error, db_close\n");
		return -1;
	}
	
	int iRet = 0;
	/*取消线程*/
	pthread_cancel(db->iPidDbExec);
	sleep(1);
