...
read_file(const char *pcPath, char *pcBuff, int iBuffSize)
{
	...
	{
		ERROR("The dir(%s) not exist.", pcPath);
		return -1;
	}
	/*打开文件*/
	fp = fopen(pcPath, "r");
...
procedure_core_get(char* pcCore)
{
	int iRet = 0;
	iRet = 
		config_read_item(GATEWAY_CONF_PATH,
							CONFIG_KEY_CORE, 
							pcCore, 
							CORE_MAX_LEN);
	if(ERROR_SUCCESS != iRet)
	{
		strncpy(pcCore, "", CORE_MAX_LEN);
	}
...
procedure_ver_get(char* pcVersion)
{
	...
	if(ERROR_SUCCESS != iRet)
	{
		strncpy(pcVersion, "", VERSION_MAX_LEN);
	}
