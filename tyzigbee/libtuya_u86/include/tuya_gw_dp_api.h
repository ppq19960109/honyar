...
typedef struct {
    int (*dev_obj_dp_report_cb)(const char *dev_id, ty_dp_s *dps, uint32_t dps_cnt);
 ...
 * @retval !0: failure
 */
int tuya_user_iot_reg_dev_data_cb(ty_dev_data_cbs_s *cbs);
 ...
 * @retval !0: failure
 */
int tuya_user_iot_send_obj_cmd(ty_obj_cmd_s *cmd);

/**
 * @brief  send raw cmd to device.
 * 
 ...
 * @retval !0: failure
 */
int tuya_user_iot_send_raw_cmd(ty_raw_cmd_s *cmd);
